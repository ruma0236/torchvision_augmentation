...
class SegmentationDataset(data.Dataset):
    ...
    def apply_transform(self, img, mask, current_transform=None):
        ...
        if isinstance(current_transform, (transforms.Compose)):
            ...
            factor = current_transform.get_params(
            ...
            if factor[1] is not None:
                transform.append(transforms.Lambda(lambda img: F.adjust_brightness(img, factor[1])))
            if factor[2] is not None:
                transform.append(transforms.Lambda(lambda img: F.adjust_contrast(img, factor[2])))
            if factor[3] is not None:
                transform.append(transforms.Lambda(lambda img: F.adjust_saturation(img, factor[3])))
            if factor[4] is not None:
                transform.append(transforms.Lambda(lambda img: F.adjust_hue(img, factor[4])))
